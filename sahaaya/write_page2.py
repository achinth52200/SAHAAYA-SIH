...
import { Building2, Users, AlertTriangle, TrendingUp, Download } from 'lucide-react';
    ...
    <div className='space-y-6'>
      ...
      <motion.section initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}>
        ...
        <div className='grid sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4'>
          {data?.districts
            .filter((d) => filterBand === 'all' || d.band_distribution[filterBand] > 0)
            .map((district, i) => (
              <div key={district.district} className='card p-5 hover:shadow-lg transition-shadow cursor-pointer' onClick={() => window.location.href = '/dashboard/districts/' + district.district}>
                ...
                <div className='flex items-center gap-3 mb-3'>
                  <span className='badge badge-red'>{district.band_distribution.Red}</span>
                  <span className='badge badge-orange'>{district.band_distribution.Orange}</span>
                  <span className='badge badge-yellow'>{district.band_distribution.Yellow}</span>
                  <span className='badge badge-green'>{district.band_distribution.Green}</span>
                </div>
                <div className='space-y-2'>
                  {[
                    { band: 'Red', count: district.band_distribution.Red, color: 'bg-red-500' },
                    { band: 'Orange', count: district.band_distribution.Orange, color: 'bg-orange-500' },
                    { band: 'Yellow', count: district.band_distribution.Yellow, color: 'bg-yellow-500' },
                    { band: 'Green', count: district.band_distribution.Green, color: 'bg-green-500' },
                  ].map((item) => (
                    <div key={item.band} className='flex items-center justify-between'>
                      ...
                      <div className='w-32 h-2 bg-gray-100 rounded-full overflow-hidden'>
                        <div className='h-full rounded-full' style={{ backgroundColor: item.color }} style={{ width: `${district.total_victims ? (item.count / district.total_victims) * 100 : 0}%` }} />
                      </div>
                    </div>
                  ))}
                </div>
